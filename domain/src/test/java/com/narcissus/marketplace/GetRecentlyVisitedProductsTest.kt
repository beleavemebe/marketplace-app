...
class GetRecentlyVisitedProductsTest {
    ...
    private val userRepository = mockk<UserRepository> {
        ...
        coEvery { isUserAuthenticated() } returns true
    }
    ...
    fun `should return actual recently visited products`() {
        runBlocking {
            Assert.assertEquals(expectedRecentlyVisitedProductsPreviews, getRecentlyVisitedProducts())
        }
        coVerifySequence { userRepository.getRecentlyVisitedProducts() }
    }
}
